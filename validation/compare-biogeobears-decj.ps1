param(
    ...
    [string]$Golden = "validation/golden/biogeobears-decj.tsv"
)
...
foreach ($case in $cases) {
    ...
    if ($delta -gt $tolerance) {
        throw "$($case.case_id): BioGeoBEARS DEC+J mismatch rust=$rustLnL biogeobears=$bgbLnL delta=$delta tolerance=$tolerance"
    }
