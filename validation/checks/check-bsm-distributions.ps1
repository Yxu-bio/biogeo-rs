...
. (Join-Path (Split-Path -Parent $PSScriptRoot) "biogeobears\rscript-portable.ps1")
...
try {
    ...
    if ($RefreshBioGeoBEARS) {
        ...
        Invoke-RScript `
            -Script "validation/biogeobears/biogeobears-bsm-distribution.R" `
            -Arguments @(
    ...
    Invoke-RScript `
        -Script "validation/biogeobears/extract-rust-bsm-distribution.R" `
        -Arguments @($rustStreamDir, $rustSamplesPath)

    Write-Host "`n== Compare independent empirical distributions =="
    Invoke-RScript `
        -Script "validation/biogeobears/compare-bsm-distributions.R" `
        -Arguments @(
            $goldenPath,
            $rustSamplesPath,
            $reportPath,
