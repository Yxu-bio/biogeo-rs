...
$scriptRoots = @(
    $PSScriptRoot,
    (Join-Path $validationRoot "biogeobears"),
    (Join-Path $validationRoot "lagrange-ng")
)
...
function Resolve-ValidationScript {
    ...
    if ($matches.Count -ne 1) {
        throw "Expected exactly one validation script named '$Name', found $($matches.Count)"
    }
    return $matches[0]
}
...
function Invoke-ScriptStep {
    ...
    Write-Host "`n== $Name =="
    & (Resolve-ValidationScript -Name $Script) @Parameters
}
