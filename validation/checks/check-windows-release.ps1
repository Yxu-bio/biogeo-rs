...
try {
    ...
    $publicExamplesCheck = Join-Path $repoRoot "validation\checks\check-public-cli-examples.ps1"
    ...
    $realDataWorkflowCheck = Join-Path $repoRoot "validation\checks\check-model-workflow-real-data.ps1"
    & $realDataWorkflowCheck -CliPath $cli -OutputRoot $realDataWorkflowOutput `
        -SkipBuild | Out-Host

    $presetModifierCheck = Join-Path $repoRoot "validation\checks\check-preset-modifier-matrix.ps1"
    & $presetModifierCheck -CliPath $cli -OutputRoot $presetModifierOutput `
        -SkipBuild | Out-Host

    $treeInputCheck = Join-Path $repoRoot "validation\checks\check-tree-input-equivalence.ps1"
    & $treeInputCheck -CliPath $cli -SkipBuild | Out-Host

    $largeStateSpaceCheck = Join-Path $repoRoot "validation\checks\check-large-state-space-resources.ps1"
    & $largeStateSpaceCheck -CliPath $cli -SkipBuild | Out-Host

    $stabilityCheck = Join-Path $repoRoot "validation\checks\check-windows-pc-stability.ps1"
    & $stabilityCheck `
        -CliPath $cli `
