param(
    [string]$Manifest = "validation/detection_profile_fixtures.tsv",
    [string]$GoldenPath = "validation/golden/biogeobears-detection-profile.tsv",
...
foreach ($row in $goldenRows) {
    if ($goldenByCase.ContainsKey($row.case_id)) {
        throw "duplicate BioGeoBEARS detection golden row: $($row.case_id)"
    }
    $goldenByCase[$row.case_id] = $row
}

$template = Get-Content -LiteralPath (Join-Path $repoRoot "examples/parameter_tables/dec.tsv")
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("biogeo-detection-" + [guid]::NewGuid().ToString("N"))
...
try {
    foreach ($case in $cases) {
        if (-not $goldenByCase.ContainsKey($case.case_id)) {
            throw "$($case.case_id): missing BioGeoBEARS detection golden row"
        }
